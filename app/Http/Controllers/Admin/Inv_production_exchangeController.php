...
class Inv_production_exchangeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = get_cols_where_p(new Inv_production_exchange(), array("*"), array("com_code" => $com_code, 'order_type' => 1), 'id', 'DESC', 10);
        if (!empty($data)) {
...
*/
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Inv_production_exchange::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)->where($field4, $operator4, $value4)->where($field5, $operator5, $value5)->where($field6, $operator6, $value6)->where('order_type', '=', 1)->orderBy('id', 'DESC')->paginate(10);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
                    $info->production_lines_name = Inv_production_lines::where('production_lines_code', $info->production_lines_code)->value('name');
