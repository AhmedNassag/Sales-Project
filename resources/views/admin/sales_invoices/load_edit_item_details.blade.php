                        ...
                        <select id="item_code_add" class="form-control select2" style="width: 100%;">
                            ...
                            @if (@isset($item_cards) && !@empty($item_cards))
                                @foreach ($item_cards as $info)
                                    <option @if ($item_data_detials['item_code'] == $info->item_code) selected="selected" @endif data-type="{{ $info->item_type }}" value="{{ $info->item_code }}">
                                        {{ $info->name }}
                                    </option>
                                @endforeach
                            @endif
                        ...
                        <select id="uom_id_Add" class="form-control select2" style="width: 100%;">
                            ...
                            @if (@isset($item_card_Data) && !@empty($item_card_Data))
                                @if ($item_card_Data['does_has_retailunit'] == 1)
                                    <option @if ($item_card_Data['uom_id'] == $item_data_detials['uom_id']) selected @endif data-isparentuom="1" value="{{ $item_card_Data['uom_id'] }}">
                                        {{ $item_card_Data['parent_uom_name'] }} (وحده اب)
                                    </option>
                                    <option @if ($item_card_Data['retail_uom_id'] == $item_data_detials['uom_id']) selected @endif data-isparentuom="0" value="{{ $item_card_Data['retail_uom_id'] }}">
                                        {{ $item_card_Data['retial_uom_name'] }} (وحدة تجزئة)
                                    </option>
                                @else
