            ...
            @if (@isset($inv_itemcard_batches) && !@empty($inv_itemcard_batches) && count($inv_itemcard_batches) > 0)
                ...
                @if ($uom_Data['is_master'] == 1)
                    @foreach ($inv_itemcard_batches as $info)
                        @if ($item_card_Data['item_type'] == 2)
                            ...
                            <option data-qunatity="{{ $info->quantity }}" data-price="{{ $info->unit_cost_price }}"
                                value="{{ $info->auto_serial }}"> عدد {{ $info->quantity * 1 }} {{ $uom_Data['name'] }}
                                انتاج {{ $info->production_date }} بتكلفة {{ $info->unit_cost_price * 1 }} للوحدة
                            </option>
                        @else
                            <option data-qunatity="{{ $info->quantity }}" data-price="{{ $info->unit_cost_price }}"
                                value="{{ $info->auto_serial }}"> عدد {{ $info->quantity * 1 }} {{ $uom_Data['name'] }}
                ...
                @else
                    ...
                    @foreach ($inv_itemcard_batches as $info)
                        ...
                        @if ($item_card_Data['item_type'] == 2)
                            //لو كان بتواريخ استهلاكي
                            <option data-qunatity="{{ $quantity }}" data-price="{{ $unit_cost_price }}"
                                value="{{ $info->auto_serial }}"> عدد {{ $quantity * 1 }} {{ $uom_Data['name'] }}
                                انتاج {{ $info->production_date }} بتكلفة {{ $unit_cost_price * 1 }} للوحدة </option>
                        @else
                            <option data-qunatity="{{ $quantity }}" data-price="{{ $unit_cost_price }}"
                                value="{{ $info->auto_serial }}"> عدد {{ $quantity * 1 }} {{ $uom_Data['name'] }}
                                بتكلفة {{ $unit_cost_price * 1 }} للوحدة </option>
                        @endif
                    @endforeach
