                        ...
                        @if ($data['is_approved'] == 0)
                            <form action="{{ route('admin.inv_production_exchange.update', $data['id']) }}" method="post">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="inv_production_order_auto_serial" id="inv_production_order_auto_serial"
                                        ...
                                        @if (@isset($Inv_production_order) && !@empty($Inv_production_order))
                                            @foreach ($Inv_production_order as $info)
                                                <option @if (old('inv_production_order_auto_serial', $data['inv_production_order_auto_serial']) == $info->auto_serial) selected="selected" @endif
                                                    value="{{ $info->auto_serial }}"> {{ $info->auto_serial }} </option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select @if ($added_counter_details > 0) disabled @endif name="production_lines_code"
                                        id="production_lines_code" class="form-control select2">
                                        ...
                                        @if (@isset($Inv_production_lines) && !@empty($Inv_production_lines))
                                            @foreach ($Inv_production_lines as $info)
                                                <option @if (old('production_lines_code', $data['production_lines_code']) == $info->production_lines_code) selected="selected" @endif
                                                    value="{{ $info->production_lines_code }}"> {{ $info->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
