...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <input type="hidden" id="ajax_show_more_detials_url"
                value="{{ route('admin.inv_production_order.show_more_detials') }}">
                                    ...
                                    @foreach ($data as $info)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    غير معتمد
                                                    <a href="{{ route('admin.inv_production_order.do_approve', $info->id) }}"
                                                        class="btn are_you_shue btn-sm  btn-success">اعتماد</a>
                                                @endif
                                            </td>
                                            <td>
                                                @if ($info->is_closed == 1)
                                                ...
                                                @else
                                                    مفتوح
                                                    @if ($info->is_approved == 1)
                                                        <a href="{{ route('admin.inv_production_order.do_closes_archive', $info->id) }}"
                                                            class="btn are_you_shue btn-sm  btn-success">اغلاق وترحيل</a>
                                                    @endif
                                                @endif
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($info->is_closed == 0)
                                                    <a href="{{ route('admin.inv_production_order.edit', $info->id) }}"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content modal-info  bg-info">
                <div class="modal-header">
                    <div class="modal-body " id="show_more_detialsModalBody" id
...
@endsection
@section('script')
    <script src="{{ asset('admin/js/inv_production_order.js') }}"></script>
@endsection
