                    ...
                    @if (!@empty($data))
                        @if ($data['is_approved'] == 0)
                            <form action="{{ route('admin.Services_orders.update', $data['id']) }}" method="post">
                                ...
                                <div class="form-group">
                                    <label> فئة الفاتورة @if ($Services_Added_Counter > 0)
                                            (لايمكن تحديث الفئة لوجود خدمات مضافة علي الفاتورة)
                                        @endif </label>
                                    <select name="order_type" id="order_type" class="form-control">

                                        @if ($Services_Added_Counter > 0)
                                            @if ($data['order_type'] == 1)
                                                <option @if (old('order_type', $data['order_type']) == 1) selected="selected" @endif
                                                    value="1"> خدمات مقدمة لنا</option>
                                            @else
                                                <option @if (old('order_type', $data['order_type']) == 2) selected="selected" @endif
                                                    value="2"> خدمات نقدمها للغير</option>
                                            @endif
                                        @else
                                            <option @if (old('order_type', $data['order_type']) == 1) selected="selected" @endif
                                                value="1"> خدمات مقدمة لنا</option>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="is_account_number" id="is_account_number" class="form-control">
                                        <option @if (old('is_account_number', $data['is_account_number']) == '') selected="selected" @endif value="">
                                            اختر الحالة</option>
                                        <option @if (old('is_account_number', $data['is_account_number']) == 1) selected="selected" @endif
                                            value="1"> نعم</option>
                                        <option @if (old('is_account_number', $data['is_account_number']) == 0) selected="selected" @endif
                                            value="0"> لا</option>
                                    </select>
                                ...
                                <div class="form-group" id="account_numberDiv"
                                    @if (old('is_account_number', $data['is_account_number']) == 0) style="display: none;" @endif>
                                    ...
                                    <select name="account_number" id="account_number" class="form-control select2">
                                        ...
                                        @if (@isset($accounts) && !@empty($accounts))
                                            @foreach ($accounts as $info)
                                                <option @if (old('account_number', $data['account_number']) == $info->account_number) selected="selected" @endif
                                ...
                                <div class="form-group" id="entity_nameDiv"
                                    @if (old('is_account_number', $data['is_account_number']) == 1) style="display: none;" @endif>
                                    <label> اسم الجهة ( طياري )</label>
                                    <input name="entity_name" class="form-control" id="entity_name" type="text"
                                        value="{{ old('entity_name', $data['entity_name']) }}">
                                    @error('entity_name')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="pill_type" id="pill_type" class="form-control">
                                        <option @if (old('pill_type', $data['pill_type']) == 1) selected="selected" @endif
                                            value="1"> كاش</option>
                                        <option @if (old('pill_type', $data['pill_type']) == 2) selected="selected" @endif
                                            value="2"> اجل</option>
                                    </select>
                                    @error('pill_type')
                                        <span class="text-danger">{{ $message }}</span>
