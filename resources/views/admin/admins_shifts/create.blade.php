                            ...
                            <select name="treasuries_id" id="treasuries_id" class="form-control ">
                                ...
                                @if (@isset($admins_treasuries) && !@empty($admins_treasuries))
                                    @foreach ($admins_treasuries as $info)
                                        <option value="{{ $info->treasuries_id }}"
                                            @if ($info->avaliable == false) disabled @endif> {{ $info->treasuries_name }}
                                            @if ($info->avaliable == false)
                                                (غير متاحه لإستخدامها حاليا مع مستخدم اخر)
                                            @endif
                                        </option>
                                    @endforeach
                                @endif
